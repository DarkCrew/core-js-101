...
function getPolynom(...args) {
  if (!args) return null;
  const exp = args.length - 1;
  return (x) => {
    let result = 0;
    args.forEach((elem, index) => {
      result += elem * x ** (exp - index);
    });
    return result;
  };
}
...
function retry(func, attempts) {
  return function checkFunc(curAttemp = attempts) {
    try {
      return func();
    } catch (e) {
      if (curAttemp > 0) {
        return checkFunc(curAttemp - 1);
      }
      return null;
    }
  };
...
function partialUsingArguments(fn, ...args1) {
  return function newPartialUsingArguments(...args) {
